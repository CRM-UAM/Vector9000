...
void speedProfile(void *a){ 
 ...
 extern long count_enc_l; 
 extern long count_enc_r;

void getEncoderStatus()
{
     long leftEncoder = count_enc_l;//robot.getEncLeftCount(); //leftEncoder = TIM2->CNT;//read current encoder ticks from register of 32 bit general purpose timer 2
     long rightEncoder = count_enc_r;//robot.getEncRightCount(); //rightEncoder = TIM5->CNT;//read current encoder ticks from register of 32 bit general purpose timer 5
...
void resetSpeedProfile(void)
{
  ...
  //Tune parameters
   ...
   accX = 0.5;// 2ticks * 100s^-1 * (30mm * pi)/(100 ticks / rev) = 188.49 mm/s^2  [ejecutando la rutina cada 10ms]
   decX = 0.5; // -282.74 mm/s^
